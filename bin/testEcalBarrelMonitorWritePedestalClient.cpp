...
//          }
          ...
          pedestals.setPedMeanG1(mean01);
          pedestals.setPedRMSG1(rms01);

          pedestals.setPedMeanG6(mean06);
          pedestals.setPedRMSG6(rms06);

          pedestals.setPedMeanG12(mean12);
          pedestals.setPedRMSG12(rms12);

          pedestals.setTaskStatus(1);

          try {
            ecid = econn->getEcalLogicID("EB_crystal_index", ism, ie-1, ip-1);
            dataset[ecid] = pedestals;
          } catch (runtime_error &e) {
            cerr << e.what() << endl;
          }
